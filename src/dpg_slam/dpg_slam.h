...
namespace dpg_slam {
    ...
    public:
    	...
    	/**
     	 ...
     	 */
    	std::vector<Eigen::Vector2f> GetActiveStaticPoints() {
		return active_static_points_;
	}
    	std::vector<Eigen::Vector2f> GetActiveAddedPoints() {
		return active_added_points_;
	}
    	// Get latest dynamic map
    	std::vector<Eigen::Vector2f> GetDynamicAddedPoints() {
		return dynamic_added_points_;
	}
    	std::vector<Eigen::Vector2f> GetDynamicRemovedPoints() {
	...
	/**
	 * Current Version of active and dynamic map points.
	 */
	std::vector<Eigen::Vector2f> active_static_points_;
	...
	ros::NodeHandle *node_handle_;
         ...
         *
         * @param current_submap_occ_grids[in]  Occupancy grids representing each one of the current pose chain nodes with the respective node number
         ...
         */
        void detectAndLabelChangesForCurrentPoseChain(const std::vector<std::pair<uint64_t, occupancyGrid>> &current_submap_occ_grids,
         ...
         *
         * @param current_node_occ_grid[in] Current pose chain node's occupancy grid with the node number of the node.
         ...
         */
	std::pair<std::vector<std::pair<uint64_t, occupancyGrid>>, occupancyGrid> computeLocalSubMap();
	 ...
	 *
	 * @param node_num  Number of the current pose chain node being considered.
	 * @param added_points at the node
	 * @param removed_points at the node
	 *
	 * @return whether or not to commit changes.
	 */
	bool computeBinScoreAndCommitLabelsForNode(const uint64_t &node_num, const std::vector<PointIdInfo> &addded_points,
